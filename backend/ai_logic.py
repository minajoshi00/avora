...
def _check_internet(force_refresh: bool = False) -> bool:
    ...
    try:
        ...
        logger.debug(
            "Internet connectivity check failed: %s",
            e,
        )
    if not result:
        try:
            ...
            logger.debug(
                "Secondary internet check failed: %s",
                e,
            )
...
def _log_ai_status():
    ...
    if GEMINI_KEY:
        print(f"    Masked: *** (length: {len(GEMINI_KEY)})")
    print()
    print(f"  GROQ_API_KEY:")
    print(f"    Exists: {bool(GROQ_KEY)}")
    if GROQ_KEY:
        print(f"    Masked: *** (length: {len(GROQ_KEY)})")
    print()
...
def process_message(user_message: str, attachments: Optional[list[dict]] = None):
    ...
    if analysis["memory_relevant"]:
        try:
            ...
            print(f"[MEMORY CONTEXT ERROR]: {err}")
